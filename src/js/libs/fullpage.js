...
export class FullPage {
	...
	styleSlider() {
		for (let index = 0; index < this.sections.length; index++) {
			...
			if (index === this.activeSectionId) {
				...
				if (section.classList.contains('_hide-header') && section.classList.contains('active-section')) {
	...
	setEvents() {
		...
		// КЛИК ДЛЯ ПЕРЕХОДА НА ДРУГИЕ СТРАНИЦЫ ======================================
		...
		if (secBlack) {
			secBlack.addEventListener("click", () => {
				this.switchingSection(1);
			});
		}
		if (secGreen) {
			secGreen.addEventListener("click", () => {
				this.switchingSection(2);
			});
		}
		if (secFruit) {
			secFruit.addEventListener("click", () => {
