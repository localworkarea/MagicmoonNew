	...
	if (document.querySelector(".icon-menu")) {
		document.addEventListener("click", function (e) {
			if (bodyLockStatus && e.target.closest('.icon-menu')) {
				...
				// Если клик был вне "icon-menu" и не внутри "header" или его дочерних элементов
				if (!e.target.closest('.menu__logo')) {
						document.documentElement.classList.remove("menu-open");
				}
		} else if (e.target.closest('.header') && e.target.closest('.menu__logo')) {
				// Если клик был внутри "header" и на "menu__logo"
				document.documentElement.classList.remove("menu-open");
		}
		});
	};
